...
var turn = 1
//if(turn = 1){
	//turn = 'X'.bold().toUpperCase();
//}
//else{
	//turn = 'O'.bold().toUpperCase();
//}
...
function toggle_move() {
	...
	document.getElementById("turn_info").innerHTML = 'Turn for: ' + map_turnplayer[turn];
}
...
function begin_play(){
	...
	click += 1;
	if(click > 1){
		alert("Game in Progress. Please 'Reset Play' to start again");
	}
	else{
	...
	if((p1.value == "") || (p2.value == ""))
	{
		...
		p2.disabled = true;
		game_started();
		document.getElementById("turn_info").innerHTML = 'Turn for: ' + map_turnplayer[turn].bold();
		//console.log(map_turnplayer[turn]);
	}
}
}
...
*/
function reset_play(){
	document.getElementById("turn_info").innerHTML = 'Game not started';
	var p1 = document.getElementById("player1_id");
	var p2 = document.getElementById("player2_id");
	p1.value = "";
	p2.value = "";
...
var map_turnplayer = {
	1: String("X").toUpperCase(),
	0: String("O").toUpperCase()
};
...
function play() {
	...
	//get input element
	var pmove = document.getElementById("move_text_id").value;
	//console.log(pmove);
	if (!table_ids.includes(pmove) || board_state[((map_letters[pmove[0]]-1) * 3) + (parseInt(pmove[1]) - 1)] != -1){
		alert("Invalid Move");
		return
	}

	// Update board_state to reflect player's turn
	board_state[((map_letters[pmove[0]]-1) - 3) + (parseInt(pmove[1]) - 1)] = turn;
	document.getElementById(pmove).innerHTML = map_turnplayer[turn];
	var update = table_ids.indexOf(pmove);
	board_state[update] = turn; //unsure
	//update turn if play continues
	checkwin();
	toggle_move();
}
	...
	if(((board_state[0] == board_state[1]) && (board_state[1] == board_state[2])) && ( board_state[1] != -1)){
		alert("Winner is: " + map_turnplayer[turn]);
		reset_play()
		}
	if(((board_state[3] == board_state[4]) && (board_state[5] == board_state[4])) && ( board_state[4] != -1)){
		alert("Winner is: " + map_turnplayer[turn]);
		reset_play()
	}
	if(((board_state[6] == board_state[7]) && (board_state[8] == board_state[7])) && ( board_state[7] != -1)){
		alert("Winner is: " + map_turnplayer[turn]);
		reset_play()
	}
	if(((board_state[0] == board_state[3]) && (board_state[6] == board_state[3])) && ( board_state[3] != -1)){
		alert("Winner is: " + map_turnplayer[turn]);
		reset_play()
	}
	if(((board_state[1] == board_state[4]) && (board_state[7] == board_state[4])) && ( board_state[4] != -1)){
		alert("Winner is: " + map_turnplayer[turn]);
		reset_play()
	}
	if(((board_state[2] == board_state[5]) && (board_state[8] == board_state[5])) && ( board_state[5] != -1)){
		alert("Winner is: " + map_turnplayer[turn]);
		reset_play()
	}
	if(((board_state[0] == board_state[4]) && (board_state[8] == board_state[4])) && ( board_state[4] != -1)){
		alert("Winner is: " + map_turnplayer[turn]);
		reset_play()
	}
if(((board_state[2] == board_state[4]) && (board_state[6] == board_state[4])) && ( board_state[4] != -1)){
		alert("Winner is: " + map_turnplayer[turn]);
		reset_play()
	}
}
